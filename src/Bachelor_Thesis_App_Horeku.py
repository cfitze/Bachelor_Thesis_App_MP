'''
 # @ Create Time: 2023-08-01 00:53:52.305297
'''
import pickle
import re
...
#initialise the app
app = dash.Dash(__name__, background_callback_manager=background_callback_manager, use_pages=True, external_stylesheets=[dbc.themes.QUARTZ, "/assets/styles_BA.css", "/assets/items_BA.css", "/assets/animated_arrow.css", "assets/smooth-arrow-animation/dist/style.css"]) #, assets_folder='assets') #dbc.themes.SPACELAB
...
DATA_PATH_PICKLE = PATH.joinpath("assets/Solextron_export").resolve()
DATA_PATH_CSV = PATH.joinpath("assets/Stromdaten").resolve()
...
def expensive_computation(dummy_trigger):
    ...
    def excel_computation():
        ...
        pickle_files_list = os.listdir(DATA_PATH_PICKLE)
        ...
        for file in pickle_files_list:
            if file.endswith('.pickle'):
                file_path_pickle = os.path.join(DATA_PATH_PICKLE, file)
        ...
        import_excel_full_path = os.path.join(DATA_PATH_PICKLE, 'BA_23FS_Curdin_Fitze_5_TSextract.pickle')
    ...
    def read_and_store_csv_files():
        ...
        csv_files = os.listdir(DATA_PATH_CSV)
        # Sort the list of files using the custom_sort_key function
        ...
        for file in csv_files:
            if file.endswith('.csv'):
                file_path = os.path.join(DATA_PATH_CSV, file)
                # Read the CSV file into a DataFrame
